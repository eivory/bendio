        ...
        case CommandGroup.BASIC:
            match m.command:
                ...
                case BasicCommand.REGISTER_NOTIFICATION:
                    # Local patch: UV-PRO firmware *does* send a reply here
                    # (typically a single status byte, e.g. 0x05 =
                    # INVALID_PARAMETER when an event class is unsupported).
                    # benlink upstream assumes there's no reply and raises
                    # ValueError, which crashes parsing of every register
                    # ack. Accept replies as raw bytes instead.
                    if m.is_reply:
                        return bf_bytes(n // 8)
